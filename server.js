...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    #app { display: none; }
    .sidebar { position: fixed; top: 0; left: 0; bottom: 0; width: 220px; background: #0d1117; color: #fff; padding: 1.5rem 0; overflow-y: auto; display: flex; flex-direction: column; }
    ...
    .sidebar-nav { flex: 1; }
    .sidebar-footer { padding: 1rem 1.5rem; border-top: 1px solid rgba(255,255,255,0.08); margin-top: auto; }
    ...
    @media (max-width: 768px) {
      .sidebar { position: relative; width: 100%; bottom: auto; height: auto; padding: 0.8rem 0; }
      .sidebar-footer { position: relative; }
      .sidebar-nav { display: flex; flex-wrap: wrap; gap: 2px; padding: 0 0.5rem; }
      .nav-item { padding: 8px 12px; font-size: 13px; border-radius: 6px; flex: 0 0 auto; }
      .nav-item.active { border-right: none; border-bottom: 2px solid #1D9E75; }
      .main { margin-left: 0; padding: 1rem; }
      .stat-grid { grid-template-columns: repeat(2, 1fr); }
      .filter-bar { gap: 8px; }
      .filter-bar input[type=date], .filter-bar select { font-size: 13px; padding: 6px 8px; }
      table { font-size: 12px; }
      th, td { padding: 8px 10px; }
      .rep-row { flex-wrap: wrap; gap: 8px; }
      .rep-stats { gap: 10px; }
      ...
      .sidebar-logo { padding: 0.8rem 1rem; }
...
<div id="app">
  ...
  <div class="main">
    ...
    <div class="page" id="page-leads">
      ...
      <div id="bulk-bar" style="display:none;background:#1D9E75;color:#fff;padding:10px 16px;border-radius:10px;margin-bottom:10px;display:none;align-items:center;justify-content:space-between;gap:12px">
        <span id="bulk-count" style="font-size:14px;font-weight:600">0 leads selected</span>
        <div style="display:flex;gap:8px">
          <button onclick="bulkDelete()" style="padding:7px 16px;background:#fff;color:#e74c3c;border:none;border-radius:6px;font-size:13px;font-weight:600;cursor:pointer">🗑 Delete Selected</button>
          <button onclick="clearSelection()" style="padding:7px 16px;background:rgba(255,255,255,0.2);color:#fff;border:none;border-radius:6px;font-size:13px;cursor:pointer">Cancel</button>
        </div>
      </div>
      <div class="table-card">
        ...
        <div style="overflow-x:auto">
          <table>
            <thead><tr>
              <th style="width:36px"><input type="checkbox" id="select-all-leads" onclick="toggleSelectAll(this)" title="Select all" /></th>
              <th></th><th>Received</th><th>Type</th><th>Name</th><th>Phone</th><th>Company</th><th>State</th><th>Qualify</th><th>Claimed By</th><th>Disposition</th><th>Notes</th>
            </tr></thead>
    ...
    <div class="page" id="page-password">
      ...
      <div class="form-card" style="max-width:480px;margin-bottom:1.5rem">
        <h3>🎯 My Rep Account</h3>
        <p style="font-size:13px;color:#888;margin-bottom:14px">Link your rep account so Switch to Claimer opens your full history and leads.</p>
        <div class="form-group" style="margin-bottom:12px">
          <label>Linked Rep Account</label>
          <select id="linked-rep-select" style="width:100%;padding:9px 12px;border:1px solid #ddd;border-radius:8px;font-size:14px">
            <option value="">— No rep account linked —</option>
          </select>
        </div>
        <button class="btn btn-primary" onclick="saveLinkedRep()">Save Linked Rep</button>
        <div class="form-msg" id="linked-rep-msg"></div>
      </div>
...
<script>
  ...
  function showPage(name, el) {
    ...
    if (name==='password') loadLinkedRep();
  }
  ...
  async function loadLeads() {
    ...
    tb.innerHTML = data.map(l=>`
      <tr>
        <td><input type="checkbox" class="lead-checkbox" value="${l.id}" onclick="updateBulkBar()" /></td>
  ...
  function updateBulkBar() {
    ...
    if (checked.length > 0) {
      ...
      count.textContent = `${checked.length} lead${checked.length > 1 ? 's' : ''} selected`;
    } else {
  ...
  async function bulkDelete() {
    ...
    if (!confirm(`Permanently delete ${checked.length} lead${checked.length > 1 ? 's' : ''}? This cannot be undone.`)) return;
    const ids = Array.from(checked).map(c => c.value);
    await Promise.all(ids.map(id => api(`/admin/leads/${id}`, { method: 'DELETE' })));
    clearSelection();
    loadLeads();
  }

  // ── LINKED REP ───────────────────────────────────────────
  async function loadLinkedRep() {
    const [repsData, linkedData] = await Promise.all([
      api('/admin/reps'),
      api('/admin/linked-rep')
    ]);
    if (!repsData || !linkedData) return;
    const select = document.getElementById('linked-rep-select');
    select.innerHTML = '<option value="">— No rep account linked —</option>';
    repsData.forEach(r => {
      const opt = document.createElement('option');
      opt.value = r.id;
      opt.textContent = r.name + ' (' + r.email + ')';
      if (r.id === linkedData.linkedRepId) opt.selected = true;
      select.appendChild(opt);
    });
  }

  async function saveLinkedRep() {
    const repId = document.getElementById('linked-rep-select').value;
    const msg = document.getElementById('linked-rep-msg');
    const data = await api('/admin/link-rep', { method: 'POST', body: JSON.stringify({ repId: repId || null }) });
    if (data?.success) {
      msg.className = 'form-msg success';
      msg.textContent = repId ? '✅ Rep account linked — Switch to Claimer will now use this account' : '✅ Rep account unlinked';
    } else {
  ...
  async function switchToClaimer() {
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        if (data.error?.includes('No rep account linked')) {
          if (confirm('You haven\'t linked a rep account yet. Go to My Account to set it up?')) {
            showPage('password', document.querySelector('.nav-item:last-of-type'));
            loadLinkedRep();
          }
        } else {
      ...
      const url = `/?switch_token=${data.token}&switch_name=${encodeURIComponent(data.repName)}&admin_mode=1&v=${Date.now()}`;
